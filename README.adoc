...
[source,java]
----
include::src/main/java/com/greglturnquist/analysis/WebSocketConfiguration.java[tag=code]
----
...
* `registerStompEndpoints()` is used to configure the endpoint on the backend for clients and server to link (`/analysis`).
...
[source,java]
----
include::src/main/java/com/greglturnquist/analysis/EventHandler.java[tag=code]
----

* `@RepositoryEventHandler(DataSet.class)` flags this class to trap events based on *dataSets*.
...
<3> Here is where the WebSocket is pointed at the application's `/analysis` endpoint.
...
The first line is the same as before, where all the dataSets are fetched from the server using page size. The second line shows an array of JavaScript objects being registered for WebSocket events, each with a `route` and a `callback`.

When a new dataSet is created, the behavior is to refresh the data set and then use the paging links to navigate to the *last* page. Why refresh the data before navigating to the end? It's possible that adding a new record causes a new page to get created. While it's possible to calculate if this will happen, it subverts the point of hypermedia. Instead of cobbling together customize page counts, it's better to use existing links and only go down that road if there is a performance-driving reason to do so.

When an dataSet is updated or deleted, the behavior is to refresh the current page. When you update a record, it impacts the page your are viewing. When you delete a record on the current page, a record from the next page will get pulled into the current one, hence the need to also refresh the current page.
...
`refreshAndGoToLastPage()` uses the familiar `follow()` function to navigate to the *dataSets* link with the *size* parameter applied, plugging in `this.state.pageSize`. When the response is received, you then invoke the same `onNavigate()` function from the last section, and jump to the *last* page, the one where the new record will be found.
...
Here, the `follow()` function is used to get to the *dataSets* link, and then the *POST* operation is applied. Notice how `client({method: 'GET' ...})` has no `then()` or `done()` like before? The event handler to listen for updates is now found in `refreshAndGoToLastPage()` which you just looked at.
...
* You published the URI of affected REST resources along with a contextual message ("/topic/newDataSet", "/topic/updateDataSet", etc.).
